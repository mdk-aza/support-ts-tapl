// ひとことで言えば、このコード（スニペット1）は、元のコード（スニペット2）と同じ目的（型検査）を果たしていますが、より高度で、関心の分離が明確な設計になっています。
 ...
 *
 * ここでの「代数 (Algebra)」とは、数学の群・環・体（Group/Ring/Field）といったものとは
 * 少し異なり、関数型プログラミングの文脈、特にCatamorphism (fold) パターンで使われる用語です。
 * 一言でいうと、「データ構造（今回ならTerm）の『作り方』に1対1で対応する『処理の仕方』を定義したオブジェクト」のことです。
 *
 * この設計パターンは、圏論（Category Theory）に由来する「F代数 (F-Algebra)」という概念に基づいています。
 *
